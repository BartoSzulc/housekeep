...
import { useLocations, useCreateLocation, useDeleteLocation } from '../../../../src/hooks/useHousehold';
...
export default function LocationsScreen() {
  ...
  const handleAdd = async () => {
    ...
    try {
      await createLocation.mutateAsync({ name: newName.trim() });
      setNewName('');
    } catch (err: any) {
      alert('Błąd', err.response?.data?.message || 'Nie udało się dodać lokalizacji.');
    }
  };

  const handleDelete = (location: Location) => {
    confirm('Usunąć?', `Usunąć "${location.name}"?`, () => deleteLocation.mutate(location.id));
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.addRow}>
        ...
        />
        <TouchableOpacity style={styles.addButton} onPress={handleAdd} disabled={createLocation.isPending}>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              <Text style={styles.emptyText}>Brak lokalizacji. Dodaj pierwszą powyżej.</Text>
            </View>
          }
        />
      )}
